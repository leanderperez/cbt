...
def cargar_reglas_mm_sqlite(nombre_archivo_csv, ruta_bd, nombre_tabla, codificacion='utf-8'):
    """
    Carga datos masivamente desde un archivo CSV (reglas Equipo-Material)
    a una tabla en una base de datos SQLite.
    ...
    try:
        ...
        with open(nombre_archivo_csv, 'r', encoding=codificacion) as archivo_csv:
            ...
            # Importante: Reemplazar 'equipo_origen_id' por el nombre real de la columna FK
            consulta_insert = f"""
                INSERT INTO {nombre_tabla} (material_origen_id, materiales_requeridos) 
                VALUES ({placeholders})
            """
...
cargar_reglas_mm_sqlite(
    nombre_archivo_csv='reglas_mm_VRF_json.csv', # Cambiado al nuevo nombre de archivo
    ruta_bd=ruta_db_django,
    nombre_tabla='app_reglamaterialmaterial' # Usamos el nombre de la tabla de reglas
)